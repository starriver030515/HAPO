...
def is_correct_strict_box(pred: str, gt: str, pause_tokens_index: Optional[list[int]] = None) -> tuple[int, Optional[str]]:
    ...
    gt = normalize_final_answer(gt)
    extracted_pred = normalize_final_answer(extracted_pred)
...
def verify(solution_str: str, answer: str, strict_box_verify: bool = False, pause_tokens_index: Optional[list[int]] = None) -> bool:
    ...
    # uncomment when training bottom entropy
    # if pred == "[INVALID]":
    #     correct, pred = is_correct_strict_box(solution_str, answer, pause_tokens_index)
    return correct, pred
...
def compute_score(
    ...
    if reward != 1.0 and pred != "[INVALID]":
        from verl.utils.reward_score.math_verify import compute_score_for_dapo
        ret_score = compute_score_for_dapo(pred, ground_truth)
        if ret_score > 0:
            reward = 1.0

    acc = True if reward > 0 else False
