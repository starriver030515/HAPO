...
from verl.trainer.ppo.core_algos import AdvantageEstimator, agg_loss, compute_entropy_h
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, config=None, entropys=None):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.HAPO:
        # Initialize the mask for GRPO calculation
        grpo_calculation_mask = data.batch["response_mask"]
        if multi_turn:
            # If multi-turn, replace the mask with the relevant part of loss_mask
            ...
            grpo_calculation_mask = data.batch["loss_mask"][:, -response_length:]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_hapo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
            index=data.non_tensor_batch["uid"],
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch.meta_info['norm_entropys'] = norm_entropys
                with _timer("step", timing_raw):
                    ...
                    with _timer("old_log_prob", timing_raw):
                        ...
                        with torch.no_grad():
                            _, norm_entropys = compute_entropy_h(entropys, response_mask=response_masks, entropy_pivot=self.config.actor_rollout_ref.actor.entropy_pivot)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer("update_actor", timing_raw):
                            ...
                            batch.meta_info['sample_data_dir'] = self.config.trainer.get("sample_data_dir", None)
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.test_freq == 0):
                        ...
                        reward_means = [
                            ...
                            and ("reward" in k or "acc" in k)
                            and re.search(r"mean@\d+$", k)
                        ]

                        cur_score = None
                        if reward_means:                     
                            cur_score = float(np.mean(reward_means))
                            val_metrics['val-core/avg/reward'] = cur_score

                        if cur_score is not None and cur_score > self.best_val_score:
                            self.best_val_score = cur_score
                            print(f"the best val score updated:{self.best_val_score}")
