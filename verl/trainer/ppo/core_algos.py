...
def compute_entropy_h(
    ...
    entropy_pivot: float = 0.5,
):
    """Return z = (entropy − median) / std (batch‑wise) where median is a
    entropy_pivot‑quantile and std the masked standard deviation.
    """
    ...
    if response_mask is None:
        response_mask = (entropy > 0).bool()
    else:
        response_mask = response_mask.bool()

    entropy = torch.log(entropy.clamp_min(eps))

    median = torch_quantile(
    ...
    N = response_mask.sum().clamp_min(1.0)
    var = (h.pow(2).masked_fill(~response_mask, 0.0).sum() / N).clamp_min(eps)
    std = var.sqrt()
    h = h / std

    pos_max = h.masked_fill(~(response_mask & (h > 0)), 0.0).max().clamp_min(eps)
    neg_min = h.masked_fill(~(response_mask & (h < 0)), 0.0).min().clamp_max(-eps)

    scale_pos = 1.0 / pos_max
    scale_neg = 1.0 / neg_min

    h_hat = torch.where(h > 0, h * scale_pos, torch.where(h <= 0, h * scale_neg, torch.zeros_like(h)))
    
    return h_hat, (median, std, scale_pos, scale_neg)
...
@register_adv_est(AdvantageEstimator.HAPO) # or simply: @register_adv_est("hapo")
def compute_hapo_outcome_advantage(
    ...
    """
    Compute advantage for HAPO, operating only on Outcome reward
...
def compute_policy_loss(
    ...
    entropy_pivot: float = 0.0,
    adaptive_clip: bool = False,
    clip_alpha: float = 1.0,
    adaptive_advantage: bool = False,
    adv_beta: float = 1.0,
    ...
    with torch.no_grad():
        h_hat, _ = compute_entropy_h(entropy, response_mask=response_mask, entropy_pivot=entropy_pivot)
        
        # hapo style adaptive clip and advantage
        if adaptive_clip:
            cliprange_low, cliprange_high = ada_clip(cliprange_low, cliprange_high, h_hat, clip_alpha)
   
        if adaptive_advantage:
            advantages = ada_advantage(advantages, h_hat, ratio, cliprange_low, cliprange_high, adv_beta)
    
    clip_low, clip_high = 1.0 - cliprange_low, 1.0 + cliprange_high
    ...
    pg_clipfrac_count = verl_F.masked_sum(torch.gt(pg_losses2, pg_losses1).float(), response_mask)
    ...
    with torch.no_grad():
        seq_lengths = response_mask.sum(dim=1).float()

        is_pos = advantages[:, 0] > 0
        is_neg = advantages[:, 0] <= 0

        sum_pos_len = seq_lengths[is_pos].sum() if is_pos.any() else torch.tensor(0.0)
        sum_neg_len = seq_lengths[is_neg].sum() if is_neg.any() else torch.tensor(0.0)
        
        avg_pos_len = seq_lengths[is_pos].mean() if is_pos.any() else torch.tensor(0.0)
        avg_neg_len = seq_lengths[is_neg].mean() if is_neg.any() else torch.tensor(0.0)
        
    return pg_loss, pg_clipfrac, pg_clipfrac_count, ratio, ppo_kl, pg_clipfrac_lower, avg_pos_len, avg_neg_len, sum_pos_len, sum_neg_len
